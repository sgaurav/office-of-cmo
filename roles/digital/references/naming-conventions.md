...
A naming scheme that survives campaign churn and feeds clean reporting. Used by `/digital` whenever a campaign, ad-set, or ad gets named.
...
This lets `/digital` (and `/analyst` on the measurement side) group performance by hook (which hooks win?) and concept (which formats win?) across audiences.
